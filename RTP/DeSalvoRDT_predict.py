...
File_genome = sys.argv[1]#input("Enter the input genome file name: ")
File_out = sys.argv[2]

fcsv = open(File_out, "a")
fcsv.write("Names_seq,RTP_RDT_region,RTP_RDT_start,RTP_RDT_end,RTP_RDT_strand,RTP_score\n",)

try:
    for seq_record in SeqIO.parse(File_genome, "fasta"):
		#MC.append(str(seq_record.seq))
		#print ("RhoTermPredict is working, please wait...")
		genome = str(seq_record.seq)#MC[0]
		genome_id = str(seq_record.id) 
		genome = genome.upper()
		...
		p = open(File_out + ".log","a")
		p.write("Sequences of predicted Rho-dependent terminators: " + genome_id)
				...
				if rapporto > 1:
					if re.search(pattern1,w):
						...
						if ctrl > 0 or re.search(pattern_pause_site1,s):
							...
							fcsv.write(','.join([genome_id,str(cod),str(s1),str(s2),'plus',str(final_score)]) + '\n')
							cod += 1    
					...
					if re.search(pattern2,w):
						...
						if ctrl > 0 or re.search(pattern_pause_site2,s):
							...
							if final_score == 0:
								final_score = score + 3
							punteggi.append(final_score)
							fcsv.write(','.join([genome_id,str(cod),str(s1),str(s2),'minus',str(final_score)]) + '\n')
							cod += 1
