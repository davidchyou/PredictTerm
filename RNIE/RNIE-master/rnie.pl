...
if(!$tabfile && $fastafile){
    ...
    my $cmsearchCmd = "$cd_path/cmsearch $cmsearchOptions --tabfile $tabfile  $modelString $fastafile > $cmsearchOut";
    print "Running:\n[$cmsearchCmd]\n" if $verbose;
...
#    open(F, "grep -v ^'#' $tabfile | sort -k6nr | ") or die "FATAL: could not open pipe for reading $tabfile\n[$!]";
  ...
  TABFILE: while (my $tabline = <F>){
	...
	if (not -s $fastafile . '.ssi'){
	    system("$cd_path/esl-sfetch --index $fastafile ") 
...
FT                    /gene=\42terminator" . $tag . "\42\n";
	...
	foreach my $cmName (@cmNames){
	    ...
	    my $sfetchCmd = "$cd_path/esl-sfetch -o ". $alnFileNames{$cmName} .".fa -C -f $fastafile " . $alnFileNames{$cmName};
	    ...
	    my $cmalignCmd = "$cd_path/cmalign -l -o $outFile $modelString " . $alnFileNames{$cmName} . '.fa';
